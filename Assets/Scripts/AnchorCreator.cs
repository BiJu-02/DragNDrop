...
public class AnchorCreator : MonoBehaviour
{
    ...
    [SerializeField]
    GameObject m_SpawnPref;

    [SerializeField]
    int m_SpawnCount;

    [SerializeField]
    Button m_BasketButton;

    [SerializeField]
    Text m_BasketStatusText;

    [SerializeField]
    Camera m_Camera;

    GameObject m_SpawnedObj;
    GameObject m_BasketObj;
    ...
    public void ButtonClickHandler()
    {
        if (this.enabled)
        {
            m_BasketStatusText.text = "basket inactive";
            this.enabled = false;
        }
        else
        {
            m_BasketStatusText.text = "basket active";
            this.enabled = true;
        }
    }
    ...
    {
        m_BasketStatusText.text = "basket active";
        m_BasketButton.onClick.AddListener(ButtonClickHandler);
    ...
    void Update()
    {
        ...
        if (m_BasketObj != null)
        {
            // check with physics raycast for spawned object selection
            if (touch.phase == TouchPhase.Began)
            {
                ...
                {
                    Debug.Log("fiziks raycasted");
                    if (hit.collider.gameObject.CompareTag("ball"))
                    {
                        m_SpawnSelected = true;
                        m_SpawnedObj = hit.collider.gameObject;
                        Debug.Log("obj selected");
                    }
                }
            }
            else if ((touch.phase == TouchPhase.Moved || touch.phase == TouchPhase.Stationary) && m_SpawnSelected)
            {
                // change position with touch
                Debug.Log("touch moved");
                if (m_RaycastManager.Raycast(touch.position, s_Hits))
                {
                    m_SpawnedObj.transform.position = s_Hits[0].pose.position;
        ...
        if (m_RaycastManager.Raycast(touch.position, s_Hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            m_BasketObj = Instantiate(m_AnchorPrefab, anchor.transform);
            for (int i = 0; i < m_SpawnCount; i++)
            {
                Instantiate(m_SpawnPref, new Vector3(anchor.transform.position.x + Random.Range(0.7f, 1.3f), anchor.transform.position.y, anchor.transform.position.z - Random.Range(0.7f, 1.3f)), Quaternion.identity);
            }
            Debug.Log("basket created");
